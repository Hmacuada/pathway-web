...
const ROADMAP = [
  { icon: Map,      name: "Flow",      desc: "Planificación de rutas y despacho",  color: "#0891b2", bg: "from-cyan-500 to-blue-600" },
  { icon: Package2, name: "Supply",    desc: "Compras, inventario y ventas",        color: "#16a34a", bg: "from-green-600 to-emerald-600" },
  { icon: BarChart3,name: "Analytics", desc: "Reportes y tendencias operativas",    color: "#dc2626", bg: "from-red-500 to-rose-600" },
];
      ...
      <main className="flex-1">
        ...
        <section className="px-6 py-16 bg-white dark:bg-slate-950">
          <div className="max-w-5xl mx-auto text-center">
            ...
            <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest mb-3">La plataforma</p>
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white mb-4">Pathway OS</h2>
            <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl mx-auto leading-relaxed mb-10">
              El entorno donde las empresas integran todas sus apps operacionales en un solo lugar.
              Control, Flow, Supply, RO y más — conectados, sin fricciones.
            </p>

            {/* Apps que vienen */}
            <div className="flex flex-wrap items-center justify-center gap-3">
              {/* Control (disponible) */}
              <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl border-2 border-violet-200 dark:border-violet-900 bg-violet-50 dark:bg-violet-950">
                <div className="w-6 h-6 rounded-lg flex items-center justify-center"
                  style={{ background: "linear-gradient(135deg,#7c3aed,#6d28d9)" }}>
                  <DollarSign size={12} color="white" />
                </div>
                <span className="text-sm font-bold text-violet-700 dark:text-violet-400">Control</span>
                <span className="text-[10px] font-bold bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-400 px-1.5 py-0.5 rounded-full">Ya disponible</span>
              </div>
              {/* RO */}
              <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 opacity-75">
                <div className="w-6 h-6 rounded-lg flex items-center justify-center"
                  style={{ background: "linear-gradient(135deg,#0d9488,#0891b2)" }}>
                  <Bot size={12} color="white" />
                </div>
                <span className="text-sm font-bold text-slate-600 dark:text-slate-400">RO</span>
              </div>
              {/* Roadmap */}
              {ROADMAP.map(p => (
                <div key={p.name} className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 opacity-60">
                  <div className={`w-6 h-6 rounded-lg bg-gradient-to-br ${p.bg} flex items-center justify-center`}>
                    <p.icon size={12} color="white" />
                  </div>
                  <span className="text-sm font-bold text-slate-500 dark:text-slate-500">{p.name}</span>
                </div>
              ))}
            </div>
          </div>
